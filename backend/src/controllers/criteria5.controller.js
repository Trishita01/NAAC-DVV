...
const score511 = asyncHandler(async (req, res) => {
  ...
  const profiles = await extended_profile.findAll({
    attributes: ['year', 'gov_students_count', 'total_students'],
    where: {
      year: { [Sequelize.Op.between]: [startDate, endDate] },
      gov_students_count: { [Sequelize.Op.not]: null },
      total_students: { [Sequelize.Op.gt]: 0 }
    },
    order: [['year', 'DESC']]
  });
  ...
  profiles.forEach(profile => {
    ...
    const govStudents = profile.gov_students_count || 0;
...
const score512 = asyncHandler(async (req, res) => {
  ...
  const profiles = await extended_profile.findAll({
    attributes: ['year', 'inst_students_count', 'total_students'],
    where: {
      year: { [Sequelize.Op.between]: [startDate, endDate] },
      inst_students_count: { [Sequelize.Op.not]: null },
      total_students: { [Sequelize.Op.gt]: 0 }
    },
    order: [['year', 'DESC']]
  });
  ...
  profiles.forEach(profile => {
    const year = profile.year;
    const instStudents = profile.inst_students_count || 0;
    const totalStudents = profile.total_students || 0;

    if (totalStudents > 0) {
      const percentage = (instStudents / totalStudents) * 100;
      yearlyPercentages[year] = percentage;
    }
  });
...
const createResponse513 = asyncHandler(async (req, res) => {
  ...
  // score 5.1.3
  const score153 = asyncHandler(async (req, res) => {
    const criteria_code = convertToPaddedFormat("1.5.3");
    const currentYear = new Date().getFullYear();
    ...
    if (!criteria) {
      throw new apiError(404, "Criteria 1.5.3 not found in criteria_master");
    }
  
    // Step 2: Fetch the latest response entry for this criteria
    const response = await Criteria153.findOne({
      attributes: ['options'],
    ...
    if (!response) {
      throw new apiError(404, "No response found for criteria 1.5.3");
    }
...
const score514 = asyncHandler(async (req, res) => {
  ...
  const profiles = await extended_profile.findAll({
    attributes: ['year', 'students_participated', 'total_students'],
    where: {
      year: { [Sequelize.Op.between]: [startDate, endDate] },
      students_participated: { [Sequelize.Op.not]: null },
      total_students: { [Sequelize.Op.gt]: 0 }
    },
    order: [['year', 'DESC']]
  });
  ...
  if (!profiles.length) {
    throw new apiError(404, "No student participation data found in the session range");
  }

  const yearlyPercentages = {};
  profiles.forEach(profile => {
    const year = profile.year;
    const studentsParticipated = profile.students_participated || 0;
    const totalStudents = profile.total_students || 0;
...
const score521 = asyncHandler(async (req, res) => {
  ...
  const studentContacts = await response_5_2_1.findAll({
    attributes: [
  ...
  if (!studentContacts.length || !outgoingStudents.length) {
    throw new apiError(404, "No student data found in the session range");
  }
